...
(function (global) {
  ...
  function optimisticReach(e) {
    ...
    // Инфотрон, под которым пусто, не лежит, а падает: считать его надо там,
    // куда он упадёт. Разлив статичен, поэтому добираем такие отдельно — если
    // достижима хоть одна клетка на пути падения, добыча достижима. Поймано
    // сторожем на «Обрыве» (#165): два инфотрона висели над пустотой, и
    // советчик объявлял провал на нулевом такте.
    for (var f = 0; f < e.tiles.length; f++) {
      if (e.tiles[f] !== T.INFOTRON || seen[f]) continue;
      var fx = f % e.w, fy = (f - fx) / e.w, hit = false;
      for (var yy = fy + 1; yy < e.h; yy++) {
        var fj = yy * e.w + fx;
        if (e.tiles[fj] !== T.EMPTY) break;
        if (seen[fj]) { hit = true; break; }
      }
      if (hit) info++;
    }
