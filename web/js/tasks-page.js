/* Обвязка отдельной страницы «Задачи».

   Страница намеренно лёгкая. Основная панель при открытии тянет показатели
   со всех площадок — а Ozon отдаёт данные не быстрее двух запросов в
   секунду, и открытие затягивается. Здесь этого нет: спрашиваем только
   оглавление задач и один рабочий список.
...
(function (global) {
  ...
  function init() {
    ['gate', 'gate-form', 'gate-password', 'gate-error', 'page', 'tasks-body',
    ...
    dom['btn-reload'].addEventListener('click', function () { global.Tasks.reload(); });
