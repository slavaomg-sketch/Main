...
(function (global) {
  ...
  function разобрать(route) {
    ...
    // означает «покажи главную в контексте этой площадки»: это полезно
    // уже сейчас и не выдаёт пустой экран за раздел.
    if (первая === 'mp') {
      return { redirect: '/', params: { mp: части[1] || '' } };
    }
    if (первая === 'catalog') {
