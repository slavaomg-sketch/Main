...
/*
 * Порядок прохождения. Номер уровня — это имя файла, и он не меняется: на нём
 * держится сохранённый прогресс. А вот порядок в массиве LEVELS — это порядок,
 * в котором уровни открываются и в котором ведёт кнопка «Следующий». Он
 * задаётся здесь, в одном месте, и меню его больше не пересортировывает.
 *
 * Первые две главы идут по плану: там каждый уровень вводит приём, и ставить
 * их по резкости значило бы требовать приём раньше, чем он показан. Дальше
 * внутри главы уровни идут по измеренной резкости: замер показал, что по дате
 * постройки в третьей главе посреди двоек стояла девятка, а шестая начиналась
 * с восьмёрки и девятки и потом падала до единицы.
 */
var PLAN_ORDER_CHAPTERS = 2;
...
out.push('/* Порядок массива — порядок прохождения; номера уровней — имена файлов. */');
